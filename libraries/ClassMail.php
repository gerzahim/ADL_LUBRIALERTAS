...
class ClassMail{
	 ...
	 * @return void
	 */
	public function sendMailSolicitudesporRecomendarTodos($listaAsesores,$cliente){
     ...
     while($row_asesores= pg_fetch_assoc($listaAsesores)) {
 
      	$mail->AddAddress($row_asesores['Email']);
        $mail->Body = '
      			PDVSA - Lubrialertas
      			Analisis de Lubricantes PDVSA
      		';
      
      		$contacto= self::GetQuitarAcentos($row_asesores['Nombres_Apellidos']);
      		$cliente= self::GetQuitarAcentos($cliente);

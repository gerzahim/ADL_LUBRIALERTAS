...
var $ez=array('fontSize'=>5); // used for storing most of the page configuration parameters
...
function Cezpdf($paper='a4',$orientation='portrait'){
	...
	if (!is_array($paper)){
		switch (strtoupper($paper)){
			...
			case 'POWER': {$size = array(0,0,792.00,612.00); break;}
...
function ezImage($image,$pad = 5,$width = 0,$resize = 'full',$just = 'center',$border = ''){
	...
	if ($type == "jpeg"){
		$this->addJpegFromFile($image,$this->ez['leftMargin'] + $pad + $offset, $this->y + $this->getFontHeight($this->ez['fontSize']) - $pad - $height,$width_original);
		//$this->addJpegFromFile($image,$this->ez['leftMargin'] + $pad + $offset, $this->y + $this->getFontHeight($this->ez['fontSize']) - $pad - $height,$width);
	}

	if ($type == "png"){
		$this->addPngFromFile($image,$this->ez['leftMargin'] + $pad + $offset, $this->y + $this->getFontHeight($this->ez['fontSize']) - $pad - $height,$width_original);

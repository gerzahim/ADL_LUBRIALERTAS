...
class BarcodeObject {
	...
	/**
	 * Crea la imagen y la almacena en la ruta especifica deacuerdo al nombre dado.
	 *
	 * @author Jose Luis Carpio. December 2008.
	 * @example self::RenderFlushObject('../imagenes/example.png');
	 * @param string $path
	 * @return void
	 */
	function RenderFlushObject ($path) {
		if (($this->mStyle & BCS_BORDER)) {
			$this->DrawBorder();
		}
		if ($this->mStyle & BCS_IMAGE_PNG) {
			ImagePng($this->mImg,$path);
		}
		elseif($this->mStyle & BCS_IMAGE_JPEG) {
			ImageJpeg($this->mImg,$path);
		}
		else __DEBUG__("FlushObject: No output type");
	}

...
//print_r( $_SESSION);
// Vista asignada
$obj_xtpl->assign_file('CONTENIDO', $obj_config->GetVar('ruta_vista').'listasolicitud'.$obj_config->GetVar('ext_vista'));
...
switch (ACCION){
	...
	case 'ver_recomendacion':
		...
		$idcertificado =$obj_modelo->ObtenerCertificado($codigom);
			...
			$parametros =$obj_modelo->ObtenerResultados($metodo);
		...
		while($fila= $obj_conexion->GetArrayInfo($parametros)){
			...
			$prueba = $obj_generico->ToUpper($fila['prueba']);
			...
			switch ($prueba) {
        		...
        		case ($prueba == 'ESTANO'):
        		   $obj_xtpl->assign('SN', $fila['resultado']);
        			break;
        		case ($prueba == 'ALUMINIO'):
        		   $obj_xtpl->assign('AL', $fila['resultado']);
        			break;
        		case ($prueba == 'NIQUEL'):
        		   $obj_xtpl->assign('NI', $fila['resultado']);
        			break;
        		case ($prueba == 'PLATA'):
        		   $obj_xtpl->assign('AG', $fila['resultado']);
        			break;
        		case ($prueba == 'MAGNESIO'):
        		...
        		case ($prueba == 'SODIO'):
        		   $obj_xtpl->assign('NA', $fila['resultado']);
        			break;
        		case ($prueba == 'POTASIO'):
        		   $obj_xtpl->assign('K', $fila['resultado']);
        			break;
        		case ($prueba == 'GLYCOL'):
        		   $obj_xtpl->assign('NI', $fila['resultado']);
        			break;
	...
	case 'modrec':
		...
		if( is_numeric($id_Muestra) && !empty($_SESSION[OPCION.'_MOD']) ){
			// Ruta actual
			...
			// Determina el valor de las filas
			$i=1;
			if($row= $obj_conexion->GetArrayInfo($recomendaciones)){
				if( ($i % 2) >0){
					$obj_xtpl->assign('estilo_fila', 'odd');
				}
				else{
				...
				//$parametros= $obj_modelo->ObtenerParametros($id_Muestra);
				//$fila= $obj_conexion->GetArrayInfo($parametros);
		...
		while($fila= $obj_conexion->GetArrayInfo($parametros)){
			...
			switch ($prueba) {
        		...
        		case ($prueba == 'PLATA'):
        		   $obj_xtpl->assign('AG', $fila['resultado']);
        			break;
        		case ($prueba == 'MANGANESO'):
        		   $obj_xtpl->assign('MN', $fila['resultado']);
        			break;
        		case ($prueba == 'SILICIO'):
        		   $obj_xtpl->assign('SI', $fila['resultado']);
        			break;
	...
	case "update":
		...
		if( is_numeric($registro) && !$obj_generico->IsEmpty($recomendacion) && !empty($_SESSION[OPCION.'_MOD']) ){
	...
	case 'buscar':
		...
		// Ruta actual
		$_SESSION['Ruta_Buscar']= $obj_generico->RutaRegreso();
	...
	case 'avanzada':
		...
		// Ruta regreso
		$obj_xtpl->assign('ruta_regreso', $_SESSION['Ruta_Buscar']);

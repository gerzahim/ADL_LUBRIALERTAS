				...
				if( count($muestras_solicitud)>0 ){
					...
					if($codigo_sigelab= $obj_sigelab->PreRegistroSolicitud($Cod_Solicitud,$motivo,$muestras_solicitud)){
						...
						if($obj_modelo_sigelab->ActualizaInfoSolicitud($codigo_sigelab,$id_solicitud)){
							...
							$transporte= $obj_generico->CleanTextDb( $obj_modelo_sigelab->GetTransportista() );
							$cliente= $obj_generico->CleanTextDb( $obj_modelo_sigelab->GetCliente($id_cliente) );
														// Aqui viene la Funcion para enviar el email de notificacion al Asesor Comercial							
							$asesor=$obj_modelo_sigelab->GetEmailAsesor($cliente['Asesor_Comercial']);	
							
							$obj_correo->sendMailSolicitudHecha($cliente['Nombre_Cliente'], $cliente['Email'], $cliente['Direccion_Cliente'], $cliente['Telefono_Oficina_Cliente'], $transporte['Nombre_Empresa'], $transporte['Persona_Contacto'], $transporte['Email'],$cantidad_muestra, $asesor['Email'], $asesor['Nombres_Apellidos']);
						
							//$obj_correo->sendMailSolicitudesRealizadasClientes($cantidad_muestra, $cliente['Nombre_Cliente'], $asesor['Email'], $asesor['Nombres_Apellidos']);
